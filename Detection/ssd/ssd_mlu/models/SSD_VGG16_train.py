...
parser.add_argument('--dataset_root', default='/dataset/VOC2007',
...
parser.add_argument('--dist_url', default='auto', type=str,
...
def main():
    ...
    if args.dist_url == "auto":
        import socket
        sock = socket.socket()
        sock.bind(('', 0))  # 绑定随机空闲端口
        free_port = sock.getsockname()[1]
        args.dist_url = f"tcp://127.0.0.1:{free_port}"
        sock.close()
        print(f"Auto-selected distributed port: {free_port}")
    ...
    ndevs_per_node = torch.mlu.device_count() if args.device == 'mlu' else torch.cuda.device_count()
...
def train(dev_id, ndevs_per_node, args):
    ...
    if args.device == 'mlu':
        torch.mlu.set_device(args.device_id)
    ...
    if args.dataset == 'COCO':
        ...
        cfg = voc
        dataset = VOCDetection(root='/dataset/VOC2007',
                               transform=SSDAugmentation(cfg['min_dim'],
                                                         MEANS))
    if args.distributed:
