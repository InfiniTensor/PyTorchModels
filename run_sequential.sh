...
#     6.  ic1        图像分类-第1组                                         共 5 模型
#     ...
#     17. ic12       图像分类-第12组                                        共 5 模型
#
# ==============================================================================
...
# ic 子组使用特殊格式: "icN|IC:模型列表|图像分类-第N组"
declare -a TEST_GROUPS=(
    ...
    "ic1|IC:alexnet convnext_tiny densenet121 densenet161 densenet169|图像分类-第1组"
    "ic2|IC:densenet201 efficientnet_b0 efficientnet_b1 efficientnet_b2 efficientnet_b3|图像分类-第2组"
    "ic3|IC:efficientnet_b4 efficientnet_b5 efficientnet_b6 googlenet inception_v3|图像分类-第3组"
    "ic4|IC:mnasnet0_5 mnasnet0_75 mnasnet1_0 mnasnet1_3 mobilenet_v2|图像分类-第4组"
    "ic5|IC:mobilenet_v3_large mobilenet_v3_small regnet_x_16gf regnet_x_1_6gf regnet_x_3_2gf|图像分类-第5组"
    "ic6|IC:regnet_x_400mf regnet_x_800mf regnet_x_8gf regnet_y_16gf regnet_y_1_6gf|图像分类-第6组"
    "ic7|IC:regnet_y_3_2gf regnet_y_400mf regnet_y_800mf regnet_y_8gf resnet101|图像分类-第7组"
    "ic8|IC:resnet152 resnet18 resnet34 resnet50 resnext101_32x8d|图像分类-第8组"
    "ic9|IC:resnext50_32x4d shufflenet_v2_x0_5 shufflenet_v2_x1_0 shufflenet_v2_x1_5 shufflenet_v2_x2_0|图像分类-第9组"
    "ic10|IC:squeezenet1_0 squeezenet1_1 vgg11 vgg11_bn vgg13|图像分类-第10组"
    "ic11|IC:vgg13_bn vgg16 vgg16_bn vgg19 vgg19_bn|图像分类-第11组"
    "ic12|IC:vit_b_16 vit_b_32 vit_l_32 wide_resnet101_2 wide_resnet50_2|图像分类-第12组"
)
...
parse_arg() {
    ...
    # "ic" 是 ic1-ic12 的别名
    if [ "$base_arg" = "ic" ] || [ "$base_arg" = "ImageClassification" ]; then
        if [ -n "$model_filter" ]; then
            # ic/resnet18 → 在所有 ic 子组中查找包含该模型的组
            local found_ic=0
            for ((i=0; i<TOTAL_DEF_GROUPS; i++)); do
                IFS='|' read -r short args cn <<< "${TEST_GROUPS[$i]}"
                if [[ "$args" == IC:* ]] && echo "${args#IC:}" | grep -qw "$model_filter"; then
                    TASKS+=("${i}:${model_filter}")
                    found_ic=1
                    break
                fi
            done
            [ $found_ic -eq 0 ] && echo -e "${COLOR_RED}错误: 模型 '$model_filter' 未在 IC 分组中找到${COLOR_NC}" && usage_exit
        else
            # ic → 添加全部 12 个 IC 子组
            for ((i=0; i<TOTAL_DEF_GROUPS; i++)); do
                IFS='|' read -r short args cn <<< "${TEST_GROUPS[$i]}"
...
for task in "${TASKS[@]}"; do
    ...
    local disp_args="$args"
    [[ "$args" == IC:* ]] && disp_args="ImageClassification"
    if [ -n "$t_filter" ]; then
        echo -e "  ${run_idx}. ${cn} → 仅 ${t_filter}"
    else
        echo -e "  ${run_idx}. ${cn} (${disp_args})"
    fi
    run_idx=$((run_idx + 1))
done
echo ""
...
for task in "${TASKS[@]}"; do
    ...
    # 检测 IC 子组: group_args 格式为 "IC:model1 model2 ..."
    local ic_models=""
    local run_domain="$group_args"
    if [[ "$group_args" == IC:* ]]; then
        ic_models="${group_args#IC:}"
        run_domain="ImageClassification"
    fi
    ...
    echo -e "${COLOR_BLUE}    域: ${run_domain}${COLOR_NC}"
    [ -n "$ic_models" ] && echo -e "${COLOR_BLUE}    IC模型: ${ic_models}${COLOR_NC}"
    ...
    if [ -n "$ic_models" ]; then
        if [ -n "$t_filter" ]; then
            IC_MODELS="$ic_models" FILTER_MODELS="$t_filter" bash run_all.sh "$MODE" ImageClassification 2>&1 | tee "$LOG_FILE"
            rc=${PIPESTATUS[0]}
        else
            IC_MODELS="$ic_models" bash run_all.sh "$MODE" ImageClassification 2>&1 | tee "$LOG_FILE"
            rc=${PIPESTATUS[0]}
        fi
    else
        if [ -n "$t_filter" ]; then
    ...
    if [ $rc -eq 0 ]; then
        ...
        if [ -n "$ic_models" ]; then
            if [ -n "$t_filter" ]; then
                echo -e "${COLOR_RED}   复测命令: IC_MODELS=\"$ic_models\" FILTER_MODELS=$t_filter bash run_all.sh $MODE ImageClassification${COLOR_NC}"
            else
                echo -e "${COLOR_RED}   复测命令: IC_MODELS=\"$ic_models\" bash run_all.sh $MODE ImageClassification${COLOR_NC}"
            fi
        else
            if [ -n "$t_filter" ]; then
...
if [ $FAIL_GROUPS -gt 0 ]; then
    ...
    for task in "${TASKS[@]}"; do
        ...
        local_ic=""
        if [[ "$group_args" == IC:* ]]; then
            local_ic="${group_args#IC:}"
        fi
        for status in "${GROUP_STATUS[@]}"; do
            if echo "$status" | grep -q "^${desc}: FAIL"; then
                if [ -n "$local_ic" ]; then
                    if [ -n "$t_filter" ]; then
                        echo -e "  ${COLOR_CYAN}IC_MODELS=\"$local_ic\" FILTER_MODELS=$t_filter bash run_all.sh $MODE ImageClassification${COLOR_NC}"
                    else
                        echo -e "  ${COLOR_CYAN}IC_MODELS=\"$local_ic\" bash run_all.sh $MODE ImageClassification${COLOR_NC}"
                    fi
                else
                    if [ -n "$t_filter" ]; then
