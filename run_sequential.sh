...
# --- 测试分组定义 ---
# "分组简称|run_all.sh参数|中文名|IC_MODELS(可选)"
...
usage_exit() {
    ...
    for g in "${TEST_GROUPS[@]}"; do
        IFS='|' read -r short args cn ic_m <<< "$g"
        if [ -n "$ic_m" ]; then
            printf "    %-3s %-6s %s (%s)\n" "$i." "$short" "$cn" "$ic_m"
        else
            printf "    %-3s %-6s %s\n" "$i." "$short" "$cn"
...
for task in "${TASKS[@]}"; do
    ...
    [ -n "$group_ic_models" ] && echo -e "${COLOR_BLUE}    IC_MODELS: ${group_ic_models}${COLOR_NC}"
    ...
    pkill -f "main.py.*imagenet" 2>/dev/null || true
    sleep 2

    # 执行 run_all.sh
    local ic_env=""
    [ -n "$group_ic_models" ] && ic_env="IC_MODELS='$group_ic_models'"
    if [ -n "$t_filter" ]; then
        env $ic_env FILTER_MODELS="$t_filter" bash run_all.sh "$MODE" $group_args 2>&1 | tee "$LOG_FILE"
        rc=${PIPESTATUS[0]}
    elif [ -n "$group_ic_models" ]; then
        IC_MODELS="$group_ic_models" bash run_all.sh "$MODE" $group_args 2>&1 | tee "$LOG_FILE"
        rc=${PIPESTATUS[0]}
    else
