...
#     6-17. ic1~ic12  ImageClassification (每组5个模型, 共60模型)
#
# ==============================================================================
...
gpu_cleanup() {
    ...
    pkill -f "main\.py" 2>/dev/null || true
    pkill -f "train\.py" 2>/dev/null || true
    pkill -f "run_train\.sh" 2>/dev/null || true
    pkill -f "run_eval\.sh" 2>/dev/null || true
    pkill -f "run_all_models" 2>/dev/null || true
    sleep 2
    # 再 SIGKILL
    ...
    pkill -9 -f "run_eval\.sh" 2>/dev/null || true
    pkill -9 -f "run_all_models" 2>/dev/null || true
}

# --- 测试分组定义 ---
# "分组简称|run_all.sh参数|中文名|IC模型列表(仅IC分组)"
declare -a TEST_GROUPS=(
    ...
    "ic1|ImageClassification|图像分类(1/12)|alexnet convnext_tiny densenet121 densenet161 densenet169"
    "ic2|ImageClassification|图像分类(2/12)|densenet201 efficientnet_b0 efficientnet_b1 efficientnet_b2 efficientnet_b3"
    "ic3|ImageClassification|图像分类(3/12)|efficientnet_b4 efficientnet_b5 efficientnet_b6 googlenet inception_v3"
    "ic4|ImageClassification|图像分类(4/12)|mnasnet0_5 mnasnet0_75 mnasnet1_0 mnasnet1_3 mobilenet_v2"
    "ic5|ImageClassification|图像分类(5/12)|mobilenet_v3_large mobilenet_v3_small regnet_x_16gf regnet_x_1_6gf regnet_x_3_2gf"
    "ic6|ImageClassification|图像分类(6/12)|regnet_x_400mf regnet_x_800mf regnet_x_8gf regnet_y_16gf regnet_y_1_6gf"
    "ic7|ImageClassification|图像分类(7/12)|regnet_y_3_2gf regnet_y_400mf regnet_y_800mf regnet_y_8gf resnet101"
    "ic8|ImageClassification|图像分类(8/12)|resnet152 resnet18 resnet34 resnet50 resnext101_32x8d"
    "ic9|ImageClassification|图像分类(9/12)|resnext50_32x4d shufflenet_v2_x0_5 shufflenet_v2_x1_0 shufflenet_v2_x1_5 shufflenet_v2_x2_0"
    "ic10|ImageClassification|图像分类(10/12)|squeezenet1_0 squeezenet1_1 vgg11 vgg11_bn vgg13"
    "ic11|ImageClassification|图像分类(11/12)|vgg13_bn vgg16 vgg16_bn vgg19 vgg19_bn"
    "ic12|ImageClassification|图像分类(12/12)|vit_b_16 vit_b_32 vit_l_32 wide_resnet101_2 wide_resnet50_2"
)
...
usage_exit() {
    ...
    for g in "${TEST_GROUPS[@]}"; do
        IFS='|' read -r short args cn ic_models <<< "$g"
        printf "    %-3s %-22s %s\n" "$i." "$short" "$cn"
        i=$((i + 1))
    done
    exit 1
...
parse_arg() {
    ...
    if [ -z "$group_idx" ]; then
        for ((i=0; i<TOTAL_DEF_GROUPS; i++)); do
            IFS='|' read -r short args cn ic_models <<< "${TEST_GROUPS[$i]}"
...
for task in "${TASKS[@]}"; do
    ...
    t_filter=""
    [[ "$task" == *:* ]] && t_filter="${task#*:}"
    IFS='|' read -r short args cn ic_models <<< "${TEST_GROUPS[$t_idx]}"
    if [ -n "$t_filter" ]; then
        echo -e "  ${run_idx}. ${cn} → 仅 ${t_filter}"
    elif [ -n "$ic_models" ]; then
        echo -e "  ${run_idx}. ${cn} (${ic_models})"
    else
...
for task in "${TASKS[@]}"; do
    ...
    IFS='|' read -r group_name group_args group_cn ic_models <<< "${TEST_GROUPS[$t_idx]}"
    ...
    [ -n "$ic_models" ] && [ -z "$t_filter" ] && echo -e "${COLOR_BLUE}    模型: ${ic_models}${COLOR_NC}"
    ...
    case "$TASK_TIMEOUT" in
        *m) max_secs=$((${TASK_TIMEOUT%m} * 60)) ;;
        *s) max_secs=${TASK_TIMEOUT%s} ;;
        *)  max_secs=$TASK_TIMEOUT ;;
    esac

    # 后台执行 run_all.sh，输出重定向到日志文件
    if [ -n "$t_filter" ]; then
        FILTER_MODELS="$t_filter" bash run_all.sh "$MODE" $group_args > "$LOG_FILE" 2>&1 &
    elif [ -n "$ic_models" ]; then
        IC_MODELS="$ic_models" bash run_all.sh "$MODE" $group_args > "$LOG_FILE" 2>&1 &
    else
        bash run_all.sh "$MODE" $group_args > "$LOG_FILE" 2>&1 &
    fi
    task_pid=$!
    # 实时显示日志
    ...
    # 等待完成或超时（ImageClassification 不限超时）
    elapsed=0
    timed_out=0
    no_timeout=0
    echo "$group_args" | grep -qw "ImageClassification" && no_timeout=1
    while kill -0 $task_pid 2>/dev/null; do
        if [ $no_timeout -eq 0 ] && [ $elapsed -ge $max_secs ]; then
            echo -e "\n${COLOR_RED}  [TIMEOUT] ${task_desc} 超时 (${TASK_TIMEOUT})，强制终止${COLOR_NC}"
            timed_out=1
            break
        fi
        sleep 2
    ...
    if [ $timed_out -eq 1 ]; then
        # 递归杀掉 run_all.sh 整棵进程树
        kill_tree $task_pid TERM
        sleep 2
        kill_tree $task_pid KILL
    ...
    # 清理残留GPU进程（timeout 后 Python 进程可能仍在占用 GPU）
    gpu_cleanup
    ...
    if [ $rc -eq 0 ]; then
        ...
        if [ -n "$t_filter" ]; then
            ...
            echo -e "${COLOR_RED}   复测命令: IC_MODELS='$ic_models' bash run_all.sh $MODE $group_args${COLOR_NC}"
        else
        ...
        for status in "${GROUP_STATUS[@]}"; do
            if echo "$status" | grep -q "^${desc}: FAIL"; then
                if [ -n "$t_filter" ]; then
                    ...
                    echo -e "  ${COLOR_CYAN}IC_MODELS='$ic_models' bash run_all.sh $MODE $group_args${COLOR_NC}"
                else
