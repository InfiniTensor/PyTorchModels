...
declare -a TEST_GROUPS=(
    ...
    "ic1|ImageClassification|IC-G1:alexnet等5模型"
    "ic2|ImageClassification|IC-G2:densenet201等5模型"
    "ic3|ImageClassification|IC-G3:efficientnet_b4等5模型"
    "ic4|ImageClassification|IC-G4:mnasnet0_5等5模型"
    "ic5|ImageClassification|IC-G5:mobilenet_v3_large等5模型"
    "ic6|ImageClassification|IC-G6:regnet_x_400mf等5模型"
    "ic7|ImageClassification|IC-G7:regnet_y_3_2gf等5模型"
    "ic8|ImageClassification|IC-G8:resnet152等5模型"
    "ic9|ImageClassification|IC-G9:resnext50_32x4d等5模型"
    "ic10|ImageClassification|IC-G10:squeezenet1_0等5模型"
    "ic11|ImageClassification|IC-G11:vgg13_bn等5模型"
    "ic12|ImageClassification|IC-G12:vit_b_16等5模型"
)

# IC 子组模型映射
declare -A IC_MODELS_MAP=(
    ["ic1"]="alexnet convnext_tiny densenet121 densenet161 densenet169"
    ["ic2"]="densenet201 efficientnet_b0 efficientnet_b1 efficientnet_b2 efficientnet_b3"
    ["ic3"]="efficientnet_b4 efficientnet_b5 efficientnet_b6 googlenet inception_v3"
    ["ic4"]="mnasnet0_5 mnasnet0_75 mnasnet1_0 mnasnet1_3 mobilenet_v2"
    ["ic5"]="mobilenet_v3_large mobilenet_v3_small regnet_x_16gf regnet_x_1_6gf regnet_x_3_2gf"
    ["ic6"]="regnet_x_400mf regnet_x_800mf regnet_x_8gf regnet_y_16gf regnet_y_1_6gf"
    ["ic7"]="regnet_y_3_2gf regnet_y_400mf regnet_y_800mf regnet_y_8gf resnet101"
    ["ic8"]="resnet152 resnet18 resnet34 resnet50 resnext101_32x8d"
    ["ic9"]="resnext50_32x4d shufflenet_v2_x0_5 shufflenet_v2_x1_0 shufflenet_v2_x1_5 shufflenet_v2_x2_0"
    ["ic10"]="squeezenet1_0 squeezenet1_1 vgg11 vgg11_bn vgg13"
    ["ic11"]="vgg13_bn vgg16 vgg16_bn vgg19 vgg19_bn"
    ["ic12"]="vit_b_16 vit_b_32 vit_l_32 wide_resnet101_2 wide_resnet50_2"
)
...
cleanup_python() {
    ...
    if [ -n "$leaked" ]; then
        echo -e "${COLOR_YELLOW}  清理残留进程: $(echo $leaked | tr '\n' ' ')${COLOR_NC}"
...
for task in "${TASKS[@]}"; do
    ...
    # 执行 run_all.sh
    ic_models="${IC_MODELS_MAP[$group_name]}"
    if [ -n "$t_filter" ]; then
        FILTER_MODELS="$t_filter" bash run_all.sh "$MODE" $group_args 2>&1 | tee "$LOG_FILE"
        rc=${PIPESTATUS[0]}
    elif [ -n "$ic_models" ]; then
        IC_MODELS="$ic_models" bash run_all.sh "$MODE" $group_args 2>&1 | tee "$LOG_FILE"
        rc=${PIPESTATUS[0]}
    else
    ...
    # 清理 timeout 后残留的 python 进程，释放 GPU 显存
    cleanup_python
