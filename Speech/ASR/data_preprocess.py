...
add_arg('annotation_path', str, '../data/annotation/', '标注文件的路径')
add_arg('train_manifest', str, '../data/manifest.train', '训练数据的数据列表路径')
add_arg('test_manifest', str, '../data/manifest.test', '测试数据的数据列表路径')
...
add_arg('dataset_vocab', str, '../data/vocabulary.txt', '生成的数据字典文件')
add_arg('num_workers', int, 8, '读取数据的线程数量')
add_arg('num_samples', int, -1, '用于计算均值和标准值得音频数量，当为-1使用全部数据')
add_arg('mean_std_path', str, '../data/mean_std.npz', '保存均值和标准值得numpy文件路径，后缀 (.npz).')
