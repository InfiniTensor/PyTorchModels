...
add_arg('dataset_vocab',    str,   './cache/vocabulary.txt',  '数据字典的路径')
add_arg('mean_std_path',    str,   './cache/mean_std.npz',    '数据集的均值和标准值的npy文件路径')
add_arg('metrics_type',     str,   'cer',                     '计算错误率方法', choices=['cer', 'wer'])
add_arg('feature_method',   str,   'linear',                  '音频预处理方法', choices=['linear', 'mfcc', 'fbank'])
add_arg('resume_model',     str,   '/data/Dataset/best_model/', '模型的路径')
