...
class DQN:
    ...
    def __init__(self, n_states, n_hidden, n_actions,
        ...
        self.q_net = Net(self.n_states, self.n_hidden, self.n_actions).to("mlu")
        # 实例化目标网络
        self.target_q_net = Net(self.n_states, self.n_hidden, self.n_actions).to("mlu")
    ...
    def take_action(self, state):
        ...
        state = torch.Tensor(state[np.newaxis, :]).to("mlu")
        # 如果小于该值就取最大的值对应的索引
    ...
    def update(self, transition_dict):  # 传入经验池中的batch个样本
        ...
        states = torch.tensor(transition_dict['states'], dtype=torch.float).to("mlu")
        # 获取当前时刻采取的动作 tuple_shape=[b]，维度扩充 [b,1]
        actions = torch.tensor(transition_dict['actions']).to("mlu").view(-1,1)
        # 当前状态下采取动作后得到的奖励 tuple=[b]，维度扩充 [b,1]
        rewards = torch.tensor(transition_dict['rewards'], dtype=torch.float).to("mlu").view(-1,1)
        # 下一时刻的状态 array_shape=[b,4]
        next_states = torch.tensor(transition_dict['next_states'], dtype=torch.float).to("mlu")
        # 是否到达目标 tuple_shape=[b]，维度变换[b,1]
        dones = torch.tensor(transition_dict['dones'], dtype=torch.float).view(-1,1).to("mlu")
